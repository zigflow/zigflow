...
// TestChildWorkflowsE2E runs the parent workflow, which calls a child workflow
// synchronously and then fans out to two children in parallel. All workflows
// run on the same task queue served by the single worker.
func TestChildWorkflowsE2E(t *testing.T) {
	ctx := t.Context()
	...
	// Parent waits 5s, calls a 10s child, fans out (max 10s), then waits 5s.
	...
	assert.Equal(t, true, got["completed"], "parent should report completed")
}
