...
func exec() error {
	...
	input := map[string]any{
		...
		"sendAt":             time.Now().Add(10 * time.Second).UTC().Format(time.RFC3339),
		"recipient":          "demo@example.com",
		"body":               "Hello from the new Zigflow wait custom wait task",
	}

	ctx := context.Background()
	we, err := c.ExecuteWorkflow(ctx, workflowOptions, "wait", input)
	if err != nil {
