...
// TestRunTaskE2E runs the run-task example, which executes a container task
// (alpine) via the worker's Docker runtime. The worker shares the host Docker
// daemon that Testcontainers already requires, so no extra infrastructure is
// needed. The container prints its environment, which is captured in the
// container task output.
func TestRunTaskE2E(t *testing.T) {
	ctx := t.Context()
	...
	assert.Contains(t, got, "container", "container task output should be present")
}
