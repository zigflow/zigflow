...
// TestRaiseE2E runs the raise example, which deliberately raises an error. The
// workflow is expected to fail, so the assertion is that execution returns an
// error rather than completing.
func TestRaiseE2E(t *testing.T) {
	ctx := t.Context()
	...
	// The workflow raises a communication error, so Get must return an error.
	var result any
	err = we.Get(runCtx, &result)
	assert.Error(t, err, "raise workflow should fail")
}
