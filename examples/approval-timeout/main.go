...
func exec() error {
	...
	we, err := c.ExecuteWorkflow(ctx, workflowOptions, "approval-timeout")
	if err != nil {
		return gh.FatalError{
			Cause: err,
	...
	go func() {
		// Approve well within the timeout. Remove this to watch the workflow
		// auto-reject once the timeout elapses.
		time.Sleep(time.Second * 2)

		log.Info().Msg("Sending approval signal")
		if err := c.SignalWorkflow(ctx, we.GetID(), "", "approve", map[string]any{
			"approver": "alice",
		}); err != nil {
