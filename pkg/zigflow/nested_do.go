...
// `then:` target is skipped because jumping to it is the intended pattern. It
...
// walkTaskItem records the task's `then:` targets and descends into every
// task list the task runs through the do builder.
func (w *nestedDoWalker) walkTaskItem(item *model.TaskItem, path string) {
	if base := item.GetBase(); base != nil && base.Then != nil {
		w.targets[base.Then.Value] = true
	}
