...
// TestDiscoverExamplesIncludesNativeExamples checks that the hermetic examples
// which opted into testing are picked up by discovery. It uses a subset check so
// adding further examples later does not break this test.
func TestDiscoverExamplesIncludesNativeExamples(t *testing.T) {
	cases, err := DiscoverExamples(examplesDir(t))
	require.NoError(t, err)

	discovered := make(map[string]bool, len(cases))
	for _, c := range cases {
		discovered[c.Name] = true
		assert.NotNil(t, c.Test, "%s must have a test runner", c.Name)
	}

	for _, want := range []string{
		"examples/hello-world",
		"examples/conditionally-execute",
		"examples/wait",
		"examples/priority-and-fairness",
		"examples/for-loop",
	} {
		assert.True(t, discovered[want], "expected %q to be discovered via its test.yaml", want)
	}
}

// TestExampleTestParsesExpectedAndAssert proves a test.yaml can declare exact
// matching and structural assertions together, and that the parsed Assert value
// is usable by CheckStructure.
func TestExampleTestParsesExpectedAndAssert(t *testing.T) {
	var et ExampleTest
...
`), &et))
	...
	assert.Equal(t, map[string]any{"foo": "bar"}, et.Input)
	assert.Equal(t, map[string]any{helloMessageKey: sampleString}, et.Expected)
	require.NotNil(t, et.Assert)

	assert.NoError(t, CheckStructure(et.Assert, map[string]any{
		keyID: sampleUUID,
	}))
	assert.Error(t, CheckStructure(et.Assert, map[string]any{
		keyID: "not-a-uuid",
	}))
}
